...
export default function Reviews() {
  ...
  // We want to show 4 items at a time on desktop.
  // We'll increment by 1 index every few seconds.
  const [startIndex, setStartIndex] = useState(0);
  const visibleCount = 4; 
  const totalCount = reviews.length;
  // If we want smooth infinite loop, usually we duplicate items or handle wrapping logic carefully.
  // For simplicity and robustness given "smooth scrolling constant animation" was mentioned before but now "change every 3-5 sec",
  // we will standard carousel slide.
  ...
  // To make it fully "infinite" smoothly without empty spaces at end, we can render a window.
  // The simplest way to handle "4 in a row" cycling is to slice the array with modulo wrapping.
  ...
  return (
    <section id="reviews" style={{ backgroundColor: "#0c0c0c", color: "white", padding: "60px 0", position: "relative" }}>
      <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "0 60px" }}> 
        {/* Padding side for arrows */}
        ...
        <div style={{ textAlign: "center", marginBottom: "50px" }}>
          ...
          <h2 style={{ fontSize: "36px", fontWeight: "300", margin: "0 0 20px 0" }}>
        ...
        <div style={{ position: "relative" }}>
            ...
            {visibleReviews.map((review, index) => (
                <div key={index} style={{ 
                    ...
                    gap: "15px",
                    minHeight: "300px",
                    fontSize: "13px" // Smaller tiles text
                }}>
                ...
                <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                    ...
                    <div>
                        <div style={{ fontWeight: "bold", fontSize: "13px", marginBottom: "0" }}>{review.name}</div>
                ...
                <div style={{ display: "flex", gap: "2px", color: "#e7711b" }}>
                    {[...Array(5)].map((_, i) => (
                        <i key={i} className="fas fa-star" style={{ fontSize: "12px" }}></i>
                    ))}
                </div>

                {/* Text */}
                <div style={{ fontSize: "13px", lineHeight: "1.4", color: "#ddd", flex: 1, overflow: "hidden" }}>
                    {review.text}
                </div>
                ...
                {review.reply && (
                    <div style={{ 
                        ...
                        position: "relative"
                         ...
                         {/* Reply Mask/Fade for design match */}
                        <div style={{ fontWeight: "bold", marginBottom: "2px", color: "#777" }}>Odpowiedź od właściciela <span style={{fontSize: "10px", fontWeight: "normal"}}>{review.time}</span></div>
                        <div style={{ overflow: "hidden", maxHeight: "40px", textOverflow: "ellipsis" }}>
                             {review.reply}
                        </div>
                    </div>
                )}
            ...
            {/* Dots Pagination (Optional based on pic, but standard for carousels) */}
             <div style={{ textAlign: "center", marginTop: "20px", display: "flex", justifyContent: "center", gap: "5px" }}>
